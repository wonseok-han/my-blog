...
원래 AWS EC2 환경에 배포 중이던 Next.js 프로젝트를 Vercel로 배포해야할 일이 생겼다. <del>(리더가 바꾸고 싶다고 시켰.. 읍읍..)</del>

프로젝트에선 `develop` - `staging` - `production`의 단계를 거쳐 운영 배포가 되는데 바로 여기서 문제가 발생했다.

**develop**과 **staging** 환경으로 배포할 때는 `Preview`로 배포되기 때문에 별로 상관 없었지만 `Production`으로 배포하자마자 너무 큰 이슈가 발생했다.
...
그래서 `Vercel-CLI`로 배포해보는 경험도 처음이었고, 일주일 내내 CI 만 뚫어지게 바라보며 삽질했던 나로서는 기록하지 않을 수가 없어 포스팅하게 되었다.
